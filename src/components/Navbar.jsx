    ...
    <div className="fixed bottom-0 font-poppins tracking-wide tab:bottom-1 left-0 right-0 z-50 max-w-md translate-x-0 mx-auto">
      <div className="flex justify-between bg-grid-white/[0.02] items-center bg-black/50 tab:border tab:rounded-xl border-t border-zinc-800/80 backdrop-blur-md">
        <Link
          href="/"
          className={clsx(
            "flex-1 flex flex-col items-center px-4 py-1.5 text-xs tab:text-base tab:py-2.5 font-medium",
            path === "/" && "text-blue-400"
          )}
        >
          <FaNoteSticky className="w-5 h-5 mb-1 tab:hidden" />
          notes
        </Link>
        <Link
          href={"/reflect"}
        ...
        >
          <RiBrainFill className="w-5 h-5 mb-1 tab:hidden" />
          reflect
        </Link>
        <Link
          href={"/search"}
        ...
        >
          <IoSearch className="w-5 h-5 mb-1 tab:hidden" />
          search
        </Link>
      </div>
    </div>
